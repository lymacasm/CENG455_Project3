...
static void queue_prioritize_tasks(QUEUE_STRUCT_PTR list)
{
	...
	while(list_itr != NULL)
	{
		...
		//printf("SCH: Priority of task is now %u\n", priority);
		if(err != MQX_OK)
		{
	...
	while (1) {
		...
		if(request_msg != NULL)
		{
			...
			case(CREATE):
				...
				new_task_tid = _task_create_blocked(0, request_msg->TASK_INFO->task_type,
			...
			case(ACTIVE_LIST):
				...
				break;
			case(OVRDUE_LIST):
				//printf("SCH: Retrieving overdue list\n");
